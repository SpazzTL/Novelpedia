<script lang="ts">
	...
	const filteredNovels = derived(
		...
		([
			...
			const filtered = $allNovels.filter((novel) => {
				...
				const matchesQuery =
					q === '' || novel.title?.toLowerCase().includes(q) || String(novel.id) === q;
				const matchesAuthor =
					$authorQuery === '' ||
				...
				const matchesExcludedTags =
					...
					!$excludedTags.some((excludedTag) => novelTagsLower.includes(excludedTag));
				...
				const matchesChapters =
					(novel.chapter_count ?? 0) >= $minChapters &&
					(novel.chapter_count ?? 0) <= $maxChapters;
				const matchesLikes =
			...
			filtered.sort((a, b) => {
				...
				switch ($sortBy) {
					case 'likes':
						...
						break;
					case 'chapters':
						result = (b.chapter_count ?? 0) - (a.chapter_count ?? 0);
						break;
					case 'views':
	...
	function exportFilteredNovels() {
		...
		const content = novelsToExport
			.map((novel) => `${novel.title}, ${novel.id}, ${novel.source || 'N/A'}`)
			.join('\n');
...
<main>
	...
	<section class="results-header">
		<div class="sort-controls">
			...
			<button
				class="secondary"
				on:click={exportFilteredNovels}
				title="Export current filtered results to a .txt file"
			>
				Export Filtered
			</button>
		</div>
		...
		<div class="pagination">
			<button on:click={() => currentPage.update((p) => p - 1)} disabled={$currentPage === 1}
				>⬅ Prev</button
			>
			...
			{#each $pagedNovels as novel (novel.id)}
				<a
					href={novel.source === 'Novelpia'
						...
						: novel.source === 'sfacg'
						? `https://book.sfacg.com/Novel/${novel.id}/`
						: '#'}
					target="_blank"
